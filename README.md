...
- **ask_user_question card bridge**: when the agent calls `ask_user_question` inside a WeCom session, the channel renders it as a Markdown message (full question and option details) plus a template card acting purely as the selector, with no truncated card surface — button cards for 2–6 single-choice options with short labels (≤6 chars, all grey as equal-weight options), vote cards for multi-select or 7–20 options with labels ≤11 chars, text cards for longer labels / larger option sets / open questions (reply with `1` or `1,3`, a label, or free text). The answer flows back and the turn continues instead of hanging.
...
- **Long tasks are neither killed nor silent**: the turn timeout is an inactivity watchdog — a turn that keeps emitting events (text deltas, tool calls, step boundaries) runs as long as it needs; only `responseTimeoutMs` with zero progress cancels it. During silent phases a heartbeat frame goes out every `streamHeartbeatMs` (default 5s, 0 disables) with cycling dots `.`/`..`/`...` and elapsed time, so the bubble visibly stays alive.
...
`/new` and `/reset` are handled directly by the WeCom plugin. It requests cancellation of the current generation, then creates a new durable session with an incrementing suffix. The old session is retained, and a service restart does not return the conversation to its old context. `/new` **keeps the current session's workspace** — it never resets the workspace choice. Other slash commands are never passed to the model as plain text. `allowedHarnessCommands` selects names that may be forwarded to the Harness command service; it defaults to `/compact`, `/goal`, and `/plan`, and a command must also be registered by the current agent preset. Because `/permission` can materially broaden agent access, enable it only together with strict `singleAllowFrom` and `groupAllowFrom` policies. `/export` depends on the Web download surface and is unavailable through WeCom. Send `/help` or `/bot-help` to list the channel commands.

Each WeCom single-chat/group session can have its **own workspace**. Send `/ws` to list the candidates (the default and current ones are marked); `/ws <number>` requests a switch — the bot first replies with a "切换 / 取消" confirmation card, and only after confirmation rotates to a new generation (a session's `meta.cwd` is immutable once created, so switching always starts a new conversation while the old history stays on the Web side); `/ws add <absolute path>` persists a new candidate through the settings service after the same double confirmation. The workspace choice is durable per session and survives restarts. The plain-text replies `1` / `确认` / `是` also settle the confirmation card; any other text cancels it. After a switch or add the plugin syncs the host workspace registry, so the Web sidebar gains a group named after the directory and the session moves into it (ordinary sessions only join existing groups and otherwise stay Ungrouped).

The scoped `wecom_send_file` tool lets the agent send an existing file when the current WeCom user asks to receive or download it. Relative paths resolve from the **current session's workspace** (switchable via `/ws`); absolute paths must also remain inside that workspace. The plugin resolves symlinks, accepts only regular files, and rejects files larger than `maxOutboundFileBytes`. The default and WeCom protocol maximum are 20,971,520 bytes (20 MiB). The tool is active only during the current WeCom turn, so continuing the same session from Web cannot send a file to the previous WeCom target. Use an allowlist whenever the configured workspace contains non-public data.
...
To verify workspaces, first configure a candidate in the settings page (or send `/ws add <absolute path>` and confirm), then send `/ws`: the bot should reply with a numbered candidate list marking the default and current entries. Send `/ws <number>`: the bot should first return a "切换 / 取消" confirmation card; after clicking "切换" (or replying `1`) it should confirm the switch and a new conversation. Sending `/ws` again should show the new "✓ 当前" mark. After switching, asking the agent to send a file inside the new workspace should succeed, while a file outside it must be rejected.
