...
- **收尾**：`finalizeReply` 挂上本回合 `wecom_send_card` 排队的卡片；`cardMode: auto` 时若回复以选项列表/确认问句结尾且没有显式卡片 → 自动生成按钮卡；所有卡片登记 `task_id → (key → 标签)` 注册表。

## 4. 卡片点击链路（`handleCardEvent`）

1. 去重 + 策略过滤；
2. **身份解析**：`cardEventFacts()` 同时兼容两种载荷形状——
   - 嵌套（平台实际格式）：`event.template_card_event.{task_id, event_key, selected_items}`
   - 平铺（SDK 类型声明）：`event.{task_id, event_key}`
3. **提问结算优先**（无任何 await 的竞态窗口）：点击命中挂起问题 → 立即结算，模型回合继续；
4. **5 秒窗口确认**：`updateTemplateCard` 原位更新卡片（提问卡 → 「已选择「label」/ 正在处理…」；普通卡 → 通用确认文案）；
5. 提问卡点击 → 额外即时回执「已收到你的选择「xx」，正在生成回复…」→ 结束（**不开新回合**）；
6. 普通卡点击 → 作为带 `task_id/event_key/选中标签/原始事件 JSON` 的用户消息注入会话，开启新回合（`ProactiveTransport` 缓冲，结束后一条 Markdown 主动推送）。
...
- **cardMode**：`auto`（回复以选项列表/确认问句结尾 → 自动按钮卡；陈述性回复不配卡）/ `tool`（仅显式发卡）/ `off`；
...
| --- | --- |
| `/bot-ping` | `pong — …已连接` |
| `/bot-card-test` | 按钮卡 → 点击 → 卡片原位变「正在处理…」→ 模型回复 |
| 长文本问题 | 文字流式逐段出现，工具执行时显示「正在执行工具 xxx…」 |
| ask 短选项问题 | Markdown + 按钮卡 → 点击 → 「已选择「xx」」→「已收到…」→ 模型继续 |
