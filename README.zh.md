...
1. 在协议要求的 **5 秒窗口内**用插件本地文案**同类型原位更新**卡片（不经过模型）：按钮卡保留原标题与全部选项按钮，选中的按钮打 ✓，标题辅助文案显示「已选择「xx」，正在处理…」；投票/多选卡锁定选项并勾选已选项；同类型更新被平台拒绝时自动回退为文本通知确认卡。`cardClickAckTitle` / `cardClickAckSubtitle` 仅在无交互面的回退场景使用；
