...
- **侧边栏分组对齐**：会话创建/resume 时向宿主 workspace 注册表对齐——显式切换/新增会 find-or-create 分组并 attach 会话（网页侧边栏不再留在未分组）；普通会话只迁入已存在的同路径分组，默认留在未分组；分组失败只记日志，不影响消息链路；
